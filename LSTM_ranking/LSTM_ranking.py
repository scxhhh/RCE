...
class LSTM_ranking(object):
    ...
    def get_cost_acc(self):
        ...
        # ranking + classify cross_entropy
        cross_entropy = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(labels=self.labels, logits=logits))+tf.nn.sigmoid_cross_entropy_with_logits(labels=self.rank, logits=logits[0]-logits[1])
    ...
    def get_cost_acc_for_test(self):
        logits = self.get_outputs()
        # ranking + classify cross_entropy
        cross_entropy = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(labels=self.labels, logits=logits))
        y_pred = tf.sigmoid(logits)
        y = self.labels
